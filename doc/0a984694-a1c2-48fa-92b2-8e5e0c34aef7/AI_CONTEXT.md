...
*   **Core**: `EventBus` (Tokio broadcast), Type definitions (`Message`, `DeviceData`).
*   **Plugins**: Wasmtime-based plugin system. Supports loading `.wasm` files and basic string/memory bridging.
...
*   **Database**: SQLite via Sea-ORM. Stores `devices` (with auth tokens), `events`, and `rules`.
*   **MQTT Broker**: Custom Ntex-based embedded broker (`flux-mqtt`).
    *   Supports v3.1.1 and v5.0 protocols.
    *   **Authentication**: Implemented `DbAuthenticator`. Connects Ntex runtime (broker) with Tokio runtime (DB access) to verify ClientID/Password.

## 2. Architecture & Design Decisions
*   **Runtime Split**: The project uses **Tokio** for the main Server/DB/EventBus, but **Ntex** for the MQTT Broker.
    *   *Hidden Knowledge*: Ntex is single-threaded/Actor-like by default. To make it work with `Sea-ORM` (which requires Tokio), we pass a `tokio::runtime::Handle` to the `DbAuthenticator` to spawn DB queries on the Tokio thread pool.
*   **Schema**:
    *   `devices`: `id` (client_id), `token` (password), `last_seen`.
    *   `rules`: `id`, `name`, `script` (Rhai code), `active`.
    *   `events`: `id`, `topic`, `payload`, `timestamp`.
...
*   **Test MQTT**: Use `test_mqtt.py` or MQTTX.
