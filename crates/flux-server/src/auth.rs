...
impl Authenticator for DbAuthenticator {
    async fn authenticate(&self, client_id: &str, _username: Option<&str>, password: Option<&[u8]>) -> Result<bool> {
        ...
        let is_valid = self.handle.spawn(async move {
            let device_opt = Devices::find_by_id(&client_id).one(&db).await.ok().flatten();
                 ...
                 if let Some(token) = &device.token {
                      if let Some(pwd) = password {
                          ...
                          if token == pwd_str {
                              ...
                              tracing::warn!("Auth failed for device {}: invalid password", client_id);
                              return false;
                          }
                      } else {
                          tracing::warn!("Auth failed for device {}: password required", client_id);
                          return false;
                      }
                 } else {
                     if password.is_some() {
                         tracing::warn!("Auth warning for device {}: password provided but none set in DB", client_id);
                     }
                     return true;
                 }
            }
